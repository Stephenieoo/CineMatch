...
class UserProfile(models.Model):
    ...
    favourite_genre1 = models.CharField(
        max_length=32, choices=Genre.choices, blank=True
    )
    favourite_genre2 = models.CharField(
        max_length=32, choices=Genre.choices, blank=True
    )
    ...
    timezone = models.CharField(
        max_length=64, blank=True, help_text="e.g., America/New_York"
    )
...
class GroupMember(models.Model):
    ...
    group_session = models.ForeignKey(
        GroupSession, on_delete=models.CASCADE, related_name="members"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
...
class GroupSwipe(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    group_session = models.ForeignKey(
        GroupSession, on_delete=models.CASCADE, related_name="swipes"
    )
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="group_swipes"
    )
    tmdb_id = models.IntegerField(db_index=True)
...
class GroupMatch(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    group_session = models.ForeignKey(
        GroupSession, on_delete=models.CASCADE, related_name="matches"
    )
    tmdb_id = models.IntegerField(db_index=True)
    ...
    class Meta:
        ...
        constraints = [
            models.UniqueConstraint(
                fields=["group_session", "tmdb_id"], name="uniq_group_match"
            )
        ]
        indexes = [
